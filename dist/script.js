...
function updateCanvas() {
  ...
  edges.forEach(edge => {
    ctx.strokeStyle = !edge.shortestPath ? "#000" : "#FFC6AC";
    if (edge.processed) {
      ctx.strokeStyle = "#FFC6AC";
    }

    if (edge.shortestPath) {
      ctx.strokeStyle = "#47A992";
    }
  ...
  points.forEach(point => {
    ...
    if (isProcessed) {
      ctx.fillStyle = "#FFC6AC";
    }
    
    if(point.processing) {
       ctx.fillStyle = "#C4C1A4";
    }

    if (optimalPath.indexOf(point.name) !== -1) {
      ctx.fillStyle = "#C4C1A4";
    }
    ctx.strokeStyle = "#9E9FA5";
    ctx.beginPath();
    ctx.arc(point.x, point.y, point.hover ? 7 : 5, 0, 2 * Math.PI);
    ctx.fill();
    ctx.stroke();
    ctx.closePath();
    ctx.fillStyle = !isProcessed ? "#000" : "#FFC6AC";
...
function calculateChilds(node, index) {
  ...
  if (!trackedCosts[child] || trackedCosts[child] > costToChild) {
    showNotification(`Calculating ${node}-${child}: ${costToChild} < ${trackedCosts[child]}, update the cost of ${child}`, animationSpeed);
    trackedCosts[child] = costToChild;
    trackedParents[child] = node;
  } else {
    showNotification(`Calculating ${node}-${child}: ${costToChild} > ${trackedCosts[child]}, maintain the cost of ${child}`, animationSpeed);
  }
...
function calculateNode() {
  ...
  showNotification(`The nearest unvisited node is ${node}`, animationSpeed);
